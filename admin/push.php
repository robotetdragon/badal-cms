...
<div class="main">
  ...
  <div class="content">
    ...
    <div class="card" style="padding:1.75rem;margin-bottom:1.25rem">
      <div class="section-label" style="margin-bottom:1rem"><?= __('push_subscribers') ?></div>
      <div style="display:flex;align-items:baseline;gap:.75rem">
        <span style="font-size:2.5rem;font-weight:800;color:var(--accent)"><?= $subCount ?></span>
        <span style="color:var(--muted);font-size:.85rem"><?= __('push_subscribers_label') ?></span>
      </div>
      <?php if ($vapidPub): ?>
    ...
    <div class="card" style="padding:1.75rem">
      <div class="section-label" style="margin-bottom:1.25rem"><?= __('push_send') ?></div>

      <?php if ($subCount === 0): ?>
        <p style="color:var(--muted);font-size:.88rem"><?= __('push_no_subscribers') ?></p>
      <?php else: ?>
        <form method="POST">
          ...
          <div class="field" style="margin-bottom:1rem">
            ...
            <textarea name="body" rows="2" placeholder="<?= e(__('push_body_placeholder')) ?>"><?= e($_POST['body'] ?? '') ?></textarea>
          </div>
          <div class="field" style="margin-bottom:1.5rem">
            <label><?= __('push_notif_url') ?></label>
            <input type="url" name="url" value="<?= e($_POST['url'] ?? $config['base_url']) ?>"
              placeholder="https://...">
