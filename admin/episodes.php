    ...
    <div class="topbar">
        ...
        <div class="topbar-actions">
            ...
            <?php if ($hasCustomOrder): ?>
                <button type="button" class="btn btn-ghost" id="btn-reset-order"
                        style="font-size:.78rem;padding:.35rem .75rem"
                        title="Revenir au tri par date">
...
<style>
    /* ── Drag handle ── */
    .drag-handle            { transition: color .15s, transform .15s; }
    .drag-handle:hover      { color: var(--accent) !important; transform: scale(1.2); }
    .drag-handle:active     { transform: scale(.95); }
    tr.dragging             { opacity: .4; transition: opacity .2s; }
    tr.drag-over td         { border-top: 2px solid var(--accent) !important; }

    /* ── Episode row: clickable feel ── */
    #episodes-body tr          { transition: background .15s; cursor: default; }
    #episodes-body tr:hover td { background: rgba(232,255,90,.03); }
    [data-theme="light"] #episodes-body tr:hover td { background: rgba(196,92,42,.03); }

    /* ── Draft badge ── */
    .badge-draft { background: rgba(255,180,50,.1); border-color: rgba(255,180,50,.25); color: #ffb432; }

    /* ── Actions: reveal on hover ── */
    .ep-actions          { opacity: .5; transition: opacity .15s; }
    tr:hover .ep-actions { opacity: 1; }
</style>
