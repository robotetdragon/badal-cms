<?php
// ═══════════════════════════════════════════════════════════════
// PODCAST — Podcast identity & settings
// ═══════════════════════════════════════════════════════════════
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['action'])) {
    ...
    if (!empty($_FILES['cover_image']['tmp_name'])) {
        ...
        if (!$ext) {
            ...
            if (move_uploaded_file($file['tmp_name'], $dest)) {
                // Check dimensions (recommended 3000x3000 min for Apple Podcasts)
    ...
    if (!$errors) {
        $success[] = "Podcast mis à jour.";
    }
}


// ═══════════════════════════════════════════════════════════════
// DATA — iTunes categories & languages
// ═══════════════════════════════════════════════════════════════
    ...
    <div class="content">
        ...
        <form method="POST" enctype="multipart/form-data">
            <div class="pod-grid" style="margin-bottom:1.25rem">
                ...
                <!-- ── Left Column: Identity & Distribution ── -->
                    ...
                    <div class="card" style="padding:1.5rem">
                        ...
                        <div class="field">
                            ...
                            <input type="text" name="podcast_title"
                                   value="<?= e($config['podcast_title'] ?? '') ?>"
                                   placeholder="Mon Super Podcast">
                        </div>
                        <div class="field">
                            <label>Auteur</label>
                            <input type="text" name="author"
                                   value="<?= e($config['author'] ?? '') ?>"
                        ...
                        <div class="field">
                            ...
                            <input type="email" name="email"
                                   ...
                                   placeholder="contact@monpodcast.com">
                        </div>
                        <div class="field" style="margin-bottom:0">
                            <label>Description</label>
                            <textarea name="podcast_description" rows="5"
                                      placeholder="Décrivez votre podcast…"
                                      style="resize:vertical"><?= e($config['podcast_description'] ?? '') ?></textarea>
                        ...
                        <div class="field">
                            ...
                            <select name="category">
                                <?php foreach ($itunesCategories as $cat): ?>
                                    <option value="<?= e($cat) ?>" <?= ($config['category'] ?? '') === $cat ? 'selected' : '' ?>>
                                        <?= e($cat) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="field">
                            ...
                            <select name="language">
                                <?php foreach ([
                                    ...
                                    <option value="<?= e($code) ?>" <?= ($config['language'] ?? '') === $code ? 'selected' : '' ?>>
                                        <?= e($label) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                ...
                <div class="card" style="padding:1.5rem; align-self:start">
                    ...
                    <?php if ($coverImg && file_exists($coverPath)): ?>
                    ...
                    <div style="font-size:.72rem; color:var(--muted); text-align:center">
                        JPG · PNG · WebP · recommandé <strong style="color:var(--text)">3000×3000px</strong>
                    </div>
                    <div style="font-size:.68rem; color:var(--muted); text-align:center; margin-top:.2rem">
                        Requis pour Apple Podcasts, Spotify, Google Podcasts
                    </div>
                </div>
