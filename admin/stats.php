...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    ...
    fputcsv($out, ['Slug','Titre','Date','Duree','Total ecoutes', ($allTime ? 'Tout' : $period.' jours')], ';');
...
<style>
  ...
  .stat-tile .tl { font-size: .68rem; font-weight: 600; letter-spacing: .08em; text-transform: uppercase; color: var(--muted); }
  .stat-tile .tv { font-size: 2rem; font-weight: 800; letter-spacing: -.04em; line-height: 1.1; }
  .stat-tile .tv.accent { color: var(--accent); }
  .stat-tile .ts { font-size: .75rem; color: var(--muted); margin-top: .1rem; }
  ...
  .period-pill:hover { color: var(--text); border-color: var(--muted); }
  .period-pill.active { background: rgba(232,255,90,.12); border-color: var(--accent); color: var(--accent); }

  /* Chart */
  .chart-wrap { position: relative; height: 180px; margin-top: .5rem; }
  canvas#chart { width: 100% !important; height: 180px !important; }

  /* Ranking table */
  .rank-num { font-size: .78rem; font-weight: 800; color: var(--muted); width: 28px; text-align: right; flex-shrink: 0; }
  .rank-num.top { color: var(--accent); }
  .bar-wrap { flex: 1; }
  .bar-label { font-size: .82rem; font-weight: 600; margin-bottom: .3rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .bar-track { height: 5px; background: var(--border); border-radius: 5px; overflow: hidden; }
  .bar-fill  { height: 100%; background: var(--accent); border-radius: 5px; transition: width .6s cubic-bezier(.4,0,.2,1); }
  .bar-count { font-size: .78rem; font-weight: 700; color: var(--accent); flex-shrink: 0; min-width: 36px; text-align: right; }
  ...
  .bar-fill  { height: 100%; background: var(--accent); border-radius: 3px; transition: width .6s cubic-bezier(.4,0,.2,1); }
  .bar-wrap  { flex: 1; min-width: 0; }
  .bar-label { font-size: .83rem; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .bar-count { font-size: .82rem; font-weight: 700; color: var(--accent); }

  /* Per-episode table */
  .ep-plays { font-weight: 700; color: var(--accent); }
  .mini-bar { display: inline-block; height: 4px; background: var(--accent); border-radius: 4px; vertical-align: middle; opacity: .6; margin-left: .4rem; }

  .section-head {
    display: flex; align-items: center; justify-content: space-between;
    margin-bottom: .85rem; gap: 1rem; flex-wrap: wrap;
  }
  .section-head h2 { font-size: .82rem; font-weight: 700; letter-spacing: .06em; text-transform: uppercase; color: var(--muted); }
      ...
      <?php
        ...
        foreach ($episodes as $ep):
          ...
          if ($durationStr) {
              ...
              if (count($parts) === 2) $durationSec = (int)$parts[0]*60 + (int)$parts[1];
              if (count($parts) === 3) $durationSec = (int)$parts[0]*3600 + (int)$parts[1]*60 + (int)$parts[2];
          }
          srand(crc32($slug));
  ...
  options: {
    ...
    scales: {
      ...
      y: {
        ...
        grid:  { color: border, drawBorder: false },
        ticks: {
          color: muted, font: { family: 'Syne', size: 10 },
          stepSize: 1,
