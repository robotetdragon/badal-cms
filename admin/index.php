...
// ═══ PROBABILISTIC CLEANUP (1 in 10 visits) ═════════════════════════════════
// Removes expired audio rate-limit files
if (mt_rand(1, 10) === 1) {
...
$latestEpisode = $episodes[0] ?? null;   // first after reverse-chronological sort
...
<div class="main">
    ...
    <div class="topbar">
        ...
        <a href="<?= e($updateInfo['url'] ?? '#') ?>"
           target="_blank" rel="noopener"
           style="display:inline-flex; align-items:center; gap:.5rem; margin-left:auto;
                  background:rgba(232,255,90,.12); border:1px solid rgba(232,255,90,.3);
                  color:var(--accent); border-radius:8px; padding:.4rem .9rem;
                  font-size:.75rem; font-weight:700; text-decoration:none">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none"
                 stroke="currentColor" stroke-width="2.5">
                <polyline points="23 4 23 10 17 10"/>
                <path d="M20.5 15a9 9 0 1 1-.5-5.5"/>
            </svg>
    ...
    <div class="content">
        ...
        <div style="margin-bottom:1.5rem; display:flex; align-items:center;
            ...
            <div style="font-size:.72rem; letter-spacing:.08em;
            ...
            <?php if (empty($episodes)): ?>
                <div style="padding:2rem; text-align:center; color:var(--muted)">
                    ...
                    <a href="<?= url('/admin/episode_new.php') ?>"
                       style="color:var(--accent)"><?= __('dash_create_first') ?></a>
                </div>
            <?php else: ?>
                <div class="ep-list">
                        ...
                        <div class="ep-row-info">
                            ...
                            <div class="ep-row-meta">
                                ...
                                <?php if (!empty($ep['duration'])): ?>
                                    · <?= e($ep['duration']) ?>
                                <?php endif; ?>
                                <span class="ep-row-plays">
                                    <?= number_format($stats->getTotal($ep['slug']), 0, ',', ' ') ?> écoutes
                                </span>
                            </div>
                        </div>
                        <a href="<?= url('/admin/episode_edit.php?slug=' . urlencode($ep['slug'])) ?>"
                           class="btn btn-ghost"
                           style="font-size:.78rem; padding:.35rem .75rem; flex-shrink:0">
                            Modifier
                        </a>
                    </div>
                    <?php endforeach; ?>
