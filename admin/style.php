...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $_SESSION['flash'] = [
        ...
        'message' => $ok ? __('app_saved') : 'Erreur lors de la sauvegarde.',
    ];
    header('Location: ' . url('/admin/style.php'));
    exit;
}
                    ...
                    <div style="display:grid;grid-template-columns:repeat(auto-fill, minmax(200px, 1fr));gap:1rem">
                        <?php foreach ($allThemes as $slug => $themeInfo): ?>
                            ...
                            <div class="theme-card"
                                 style="<?= $borderStyle ?>;border-radius:10px;overflow:hidden;cursor:pointer;transition:border .2s,transform .2s,box-shadow .2s"
                                 data-slug="<?= e($slug) ?>"
