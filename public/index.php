...
foreach ($socials as $net => $info) {
    ...
    if ($url) {
        $href = $net === 'email' ? 'mailto:' . $url : $url;
        $socialLinks[$net] = ['url' => $href, 'label' => $info['label']];
    }
}
...
function socialIcon(string $net): string {
    $icons = [
        ...
        'email'      => '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="15" height="15"><rect x="2" y="4" width="20" height="16" rx="2"/><polyline points="22,4 12,13 2,4"/></svg>',
    ];
    return $icons[$net] ?? '🔗';
}
?>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .footer-logo-wrap {
            ...
            mask: url('<?= url('/audio/badal_logo.svg') ?>') center/contain no-repeat;
            aspect-ratio: 541/183; opacity: .7; transition: opacity .2s;
        }
        ...
        .cta-btn:hover  { color: var(--accent); border-color: var(--accent); box-shadow: 0 0 0 2px rgba(<?= $accentRgb ?>,.1); }
        .cta-btn:active { transform: scale(.95); transition-duration: .06s; box-shadow: none; }
        ...
        .social-link:hover  { color: var(--accent); border-color: var(--accent); background: rgba(<?= $accentRgb ?>,.08); transform: translateY(-2px); }
        .social-link:active { transform: scale(.9); transition-duration: .06s; }
        ...
        .episode-card-list:hover  { border-color: rgba(<?= $accentRgb ?>,.4); box-shadow: 0 2px 16px rgba(0,0,0,.15); transform: translateX(4px); }
        .episode-card-list:active { transform: scale(.99); box-shadow: none; transition-duration: .06s; }
        ...
        .ep-play-btn:hover  { opacity: .88; transform: translateY(-1px); box-shadow: 0 4px 12px rgba(0,0,0,.2); }
        .ep-play-btn:active { transform: scale(.88); box-shadow: none; transition-duration: .06s; }
        ...
        .episode-card-grid:active { transform: scale(.98); box-shadow: 0 2px 8px rgba(0,0,0,.1); transition-duration: .06s; }
        ...
        .ep-cover-wrap-placeholder     { display: flex; align-items: center; justify-content: center; }
        ...
        .featured-play:hover  { opacity: .88; transform: translateY(-1px); box-shadow: 0 6px 20px rgba(0,0,0,.2); }
        .featured-play:active { transform: scale(.95); box-shadow: none; transition-duration: .06s; }
        ...
        .share-btn:hover  { color: var(--accent); transform: scale(1.1); }
        .share-btn:active { transform: scale(.9); transition-duration: .06s; }
        ...
        .push-bell:hover    { color: var(--accent); transform: scale(1.15); }
        .push-bell:active   { transform: scale(.9); transition-duration: .06s; }
        ...
        @media (max-width: 768px) {
            ...
            .featured-cover,
            .featured-cover-placeholder { width: 100%; height: auto; aspect-ratio: 1; }
        ...
        @media (max-width: 640px) {
            ...
            /* List -> vertical cards on mobile */
            .episodes-list { display: grid; grid-template-columns: repeat(2, 1fr); gap: .6rem; }
            .episode-card-list {
                flex-direction: column; align-items: stretch;
                border-radius: 12px; padding: 0; gap: 0;
            }
            .ep-cover-list,
            .ep-cover-list-placeholder {
            ...
            .ep-cover-list { object-fit: cover; }
            .ep-list-body  { padding: .6rem .7rem .5rem; }
            .ep-play-btn {
            ...
            .episode-card-list { position: relative; }
...
// ── DOM ready: feather icons & page transitions ──────────────────────
